...
//! use colliflower::ContextManager;
//!
...
where K: Hash + Eq, S: BuildHasher {
    ...
    /// The set of all keys present in at least one map in the manager.
    ///
    /// # Example
    /// ```
    ...
    /// manager.push(HashMap::from([("y", 4), ("z", 3)]));
    ///
    /// let expected_keys = HashSet::from([&"w", &"x", &"y", &"z"]);
    /// let actual_keys = manager.key_set();
    ///
...
where K: Hash + Eq + Clone, V: Clone, S: BuildHasher + Clone {
    /// Aggregates all contexts into a single map where keys have their most recent value.
    ///
    /// The keys, values, and hasher of the resulting map, will be cloned from the
    /// underlying maps. This method is similar to [ContextManager::collapse], but doesn't
    /// consume the manager.
    ///
    /// # Example
    /// ```
    ...
    /// manager.push(HashMap::from([("y", 4), ("z", 3)]));
    ///
    /// let map = manager.current_state().unwrap();
    ///
    /// assert_eq!(&map["w"], &1);
    /// assert_eq!(&map["x"], &2);
    /// assert_eq!(&map["y"], &4);
    /// assert_eq!(&map["z"], &3);
    /// ```
    pub fn current_state(&self) -> Option<HashMap<K, V, S>> {
        if self.inner.is_empty() {
            None
        } else {
            let clen = self.inner.len();
            let mut map = self.inner[clen - 1].clone();

            for i in 0..(clen - 1) {
                let next = self.inner[clen - 2 - i].clone();
    ...
    pub fn push_with_local(&mut self, context: HashMap<K, V, S>) {
        if self.inner.is_empty() {
            ...
            let mut local_context = self.inner[0].clone();

            local_context.extend(context);

            self.inner.push_front(local_context);
        }
    }
}
